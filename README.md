...
- **Skills** — this repo is a Claude Code plugin: `/quno:quest`, `/quno:start` (reads, asks until the idea is solid, then asks implement / investigate / brief only / drop), `/quno:parse`, `/quno:investigation`, `/quno:adr`. `/learn` stays global.
